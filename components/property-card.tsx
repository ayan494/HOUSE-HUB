...
export function PropertyCard({ property, onBookClick }: PropertyCardProps) {
  ...
  return (
    <Card className="group overflow-hidden bg-card border-border hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 flex flex-col h-full p-0 gap-0">
      {/* Image Container */}
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
            {property.images.map((_, idx) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all ${idx === imageIndex ? 'bg-white w-4' : 'bg-white/60'
                  }`}
              />
            ))}
          </div>
      ...
      <CardContent className="p-3 sm:p-4 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border mt-auto gap-2">
          ...
          <Button
            size="sm"
            className="text-xs sm:text-sm shadow-md hover:shadow-lg hover:scale-105 transition-all duration-300 active:scale-95"
            onClick={(e) => {
              e.preventDefault()
